...
void draw() {
  ...
  //noFill();
  //for (int i = 0; i <= 5; i++) {
  //  ellipse(110 + i * 50, height/2, arduino.analogRead(i) / 20, arduino.analogRead(i) / 20);
  //}
  ...
  for (int i = 0; i <= 5; i++) {
    ...
    } else if (arduino.digitalRead(i) == Arduino.LOW){
      
      fill(random(255), random(255), random(255));
      ellipse(width/2, height/2, arduino.analogRead(i) / 5, arduino.analogRead(i) / 5);
    }
  }
}